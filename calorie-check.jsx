...
const FONT_IMPORT = `@import url('https://fonts.googleapis.com/css2?family=Oswald:wght@400;500;600;700&family=Source+Serif+4:opsz,wght@8..60,400;8..60,600&display=swap');`;
...
export default function CalorieJournal() {
  ...
  const loadToday = useCallback(async () => {
    try {
      const res = await window.storage.get(`foodlog:${todayStr()}`);
  ...
  const loadWeekly = useCallback(async () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      let sum = 0;
      try {
        const res = await window.storage.get(`foodlog:${ds}`);
        if (res) {
  ...
  async function saveProfile() {
    ...
    try {
      await window.storage.set("profile", JSON.stringify(profileDraft));
    } catch {
  ...
  async function fillDummyData() {
    ...
    try {
      if (!profile) {
        await window.storage.set("profile", JSON.stringify(DUMMY_PROFILE));
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        await window.storage.set(`foodlog:${ds}`, JSON.stringify(dayEntries));
      }
  ...
  async function addEntry() {
    ...
    setLoading(false);
    try {
      await window.storage.set(`foodlog:${todayStr()}`, JSON.stringify(next));
    } catch {
